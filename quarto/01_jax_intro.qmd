...
\pause
```{python}
p_hat = jr.normal(key_p, shape=(2,)) # random initial param
loss_fn(p_hat, y, x)
```
...
$$
\nabla_{1} \mathcal{L}(p, y, x): \mathbb{R}^{n_p} \times \mathbb{R}^N \times \mathbb{R}^N \mapsto \mathbb{R}^{n_p} = \frac{\partial \mathcal{L}(p, y, x)}{\partial p},
$$
...
\pause
The function ``grad_fn`` can be called with new arguments:
```{python}
grad_fn(p_hat, y, x)
```
