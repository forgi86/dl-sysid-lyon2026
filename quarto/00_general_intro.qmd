...
* Neural state-space/recurrent neural networks
...
\pause
\begin{align*}
{y} = W_3\sigma\big(W_2\sigma(W_1 x + b_1)+ b_2\big) + b_3 = \mathrm{FF}(x;\theta), \qquad \theta = \mathrm{vec}(W_1, b_2, W_2, b_2, W_3, b_3)
\end{align*}

 <!-- $$ \theta = (W_1, b_2, W_2, b_2, W_3, b_3) $$ -->

\pause
* Linear blocks interleaved by non-linear **activation functions** $\sigma$, e.g. $\tanh$ \pause
* Non-linearities $\sigma(\cdot)$ essential for **expressiveness**

## Gradient Descent

:::: {.columns}
::: {.column width="65%"}
1. Initialize randomly: $\hat \theta$
 ...
 * Used by Google to train the **Gemini** Language Model (by Anthropic for Claude?)
 * Very efficient for system identification & scientific ML
