...
\pause
```{python}
# build a single function that return both loss and its gradient
loss_grad_fn = jax.value_and_grad(loss_fn, 0)

# ask jax to compile this (it's the computational bottleneck)
...
```
\pause
* Use Adam from `optax` instead of plain gradient descent from scratch
* Save loss for diagnostics
